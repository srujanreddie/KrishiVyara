...
import { translateCrop, translateActivity } from '../utils/i18n';
import { 
  BookOpen, 
  Plus, 
  Calendar, 
...
const activityConfig: Record<ActivityType, { icon: string; color: string; badgeBg: string }> = {
  planting: { icon: '🌱', color: 'text-emerald-700', badgeBg: 'bg-emerald-100' },
  watering: { icon: '💧', color: 'text-blue-700', badgeBg: 'bg-blue-100' },
  fertilizer: { icon: '🧪', color: 'text-amber-700', badgeBg: 'bg-amber-100' },
  pesticide: { icon: '🚿', color: 'text-purple-700', badgeBg: 'bg-purple-100' },
  weeding: { icon: '✂️', color: 'text-stone-700', badgeBg: 'bg-stone-100' },
  harvest: { icon: '🌾', color: 'text-yellow-700', badgeBg: 'bg-yellow-100' },
  pest_sighting: { icon: '🐛', color: 'text-rose-700', badgeBg: 'bg-rose-100' },
  soil_treatment: { icon: '🪵', color: 'text-amber-900', badgeBg: 'bg-amber-50' },
  note: { icon: '📝', color: 'text-stone-700', badgeBg: 'bg-stone-100' }
};
...
export const FarmDiary: React.FC<FarmDiaryProps> = ({
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const actInfo = translateActivity(formActivity, userProfile.languagePreference);
    onAddEntry({
      cropName: formCrop,
      plotName: formPlot,
      ...
      notes: formNotes || `${actInfo.label} on ${translateCrop(formCrop, userProfile.languagePreference)}`,
    ...
    <div className="space-y-5 pb-24 max-w-4xl mx-auto">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              <span className="text-xs text-emerald-200 font-bold">
                {diaryEntries.length} {t.totalActivitiesSaved}
              </span>
            </div>
      ...
      <div className={`p-5 sm:p-6 rounded-[36px] border ${
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <button
            onClick={() => onAddEntry({ activityType: 'watering', cropName: userProfile.primaryCrops[0] || 'Tomato', notes: 'Standard morning drip irrigation completed.', quantity: 3, unit: 'hours', status: 'completed' })}
          ...
          <button
            onClick={() => onAddEntry({ activityType: 'fertilizer', cropName: userProfile.primaryCrops[0] || 'Tomato', notes: 'Applied NPK fertigation via drip.', quantity: 5, unit: 'kg', status: 'completed' })}
          ...
          <button
            onClick={() => onAddEntry({ activityType: 'pesticide', cropName: userProfile.primaryCrops[0] || 'Tomato', notes: 'Preventative medicine spray completed.', quantity: 2, unit: 'pumps (30L)', status: 'completed' })}
          ...
          <button
            onClick={() => onAddEntry({ activityType: 'planting', cropName: userProfile.primaryCrops[0] || 'Cotton', notes: 'Sowed seeds in plot with organic basal compost.', status: 'completed' })}
      ...
      <div className="flex items-center justify-between gap-2 overflow-x-auto pb-1 scrollbar-none">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-black text-slate-500">{t.filterByCrop}:</span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {userProfile.primaryCrops.map((crop) => (
            ...
            >
              {translateCrop(crop, userProfile.languagePreference)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          {filteredEntries.length > 0 && (
            <button
              onClick={() => {
                const summary = filteredEntries.map(e => `${e.date}: ${translateActivity(e.activityType, userProfile.languagePreference).label} on ${translateCrop(e.cropName, userProfile.languagePreference)}. ${e.notes}`).join('. ');
                handleSpeak(summary);
              }}
        ...
        {filteredEntries.length === 0 ? (
          ...
          <div className="relative pl-6 space-y-4 before:absolute before:left-2.5 before:top-3 before:bottom-3 before:w-0.5 before:bg-emerald-200">
            {filteredEntries.map((entry) => {
              const actInfo = translateActivity(entry.activityType, userProfile.languagePreference);
              const actStyle = activityConfig[entry.activityType] || activityConfig.note;
              return (
                <div
                  key={entry.id}
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        <span className={`text-[11px] font-black px-3 py-1 rounded-full ${actStyle.badgeBg} ${actStyle.color}`}>
                          {actInfo.icon} {actInfo.label}
                        </span>
                        <span className="text-xs font-black px-2.5 py-0.5 rounded-xl bg-slate-100 text-slate-800">
                          {translateCrop(entry.cropName, userProfile.languagePreference)}
                        </span>
                        {entry.plotName && (
                    ...
                    <div className="mt-3.5 pt-3 border-t border-slate-100 flex items-center gap-3 text-xs font-bold text-slate-700 flex-wrap">
                      {entry.quantity && (
                        <span className="p-2 px-3 rounded-2xl bg-slate-50 border border-slate-200 text-slate-800">
                          {t.quantityDose}: <strong className="font-black">{entry.quantity} {entry.unit}</strong>
                        </span>
                      )}
                      {entry.chemicalUsed && (
                        <span className="p-2 px-3 rounded-2xl bg-purple-50 border border-purple-200 text-purple-950 font-bold">
                          {t.chemicalProductUsed}: <strong className="font-black">{entry.chemicalUsed}</strong>
                        </span>
                      )}
                    </div>
                  )}
                  ...
                  <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between">
                    <button
                      onClick={() => handleSpeak(`${actInfo.label} on ${translateCrop(entry.cropName, userProfile.languagePreference)}. ${entry.notes}`)}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-[36px] p-7 max-w-lg w-full space-y-4 shadow-2xl border border-slate-200 my-8">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-3.5">
              <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    {userProfile.primaryCrops.map(c => (
                      <option key={c} value={c}>
                        {translateCrop(c, userProfile.languagePreference)}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 rounded-2xl border border-slate-300 text-xs font-bold bg-slate-50 focus:ring-2 focus:ring-emerald-500 font-medium"
                  >
                    <option value="watering">💧 {translateActivity('watering', userProfile.languagePreference).label}</option>
                    <option value="fertilizer">🧪 {translateActivity('fertilizer', userProfile.languagePreference).label}</option>
                    <option value="pesticide">🚿 {translateActivity('pesticide', userProfile.languagePreference).label}</option>
                    <option value="planting">🌱 {translateActivity('planting', userProfile.languagePreference).label}</option>
                    <option value="pest_sighting">🐛 {translateActivity('pest_sighting', userProfile.languagePreference).label}</option>
                    <option value="weeding">✂️ {translateActivity('weeding', userProfile.languagePreference).label}</option>
                    <option value="harvest">🌾 {translateActivity('harvest', userProfile.languagePreference).label}</option>
                    <option value="soil_treatment">🪵 {translateActivity('soil_treatment', userProfile.languagePreference).label}</option>
                    <option value="note">📝 {translateActivity('note', userProfile.languagePreference).label}</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  {t.chemicalProductUsed}:
                </label>
                <input
                  type="text"
                  value={formChemical}
                  onChange={(e) => setFormChemical(e.target.value)}
                  placeholder="e.g. Mancozeb 75% WP / NPK"
                  className="w-full px-3.5 py-2.5 rounded-2xl border border-slate-300 text-xs text-slate-900 bg-slate-50 font-medium"
                />
              </div>
